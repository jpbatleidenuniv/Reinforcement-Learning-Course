...
def sample_monte_carlo(
    ...
    while not (truncated or terminated):
        action, ratio  = policy_agent.select_action(obs)
...
def PPO(
    ...
    with tqdm(total=max_steps, unit="steps") as pbar:
        while total_steps < max_steps:
            ...
            advantage = value_agent.advantage()
            policy_info = policy_agent.update(advantage=advantage)
    ...
    if save_plot:
        if iteration is None:
            fig.savefig(save_plot / f"{config.run.name}_training.png") # type: ignore
        else:
            fig.savefig(save_plot / f"{config.run.name}_{iteration}_training.png") # type: ignore
    return eval_history
...
if __name__ == "__main__":
    ...
    PPO(config=cfg, env=env, save_plot=save_path, iteration=1)