...
class StochasticWindyGridworld:
    ...
    def __init__(self, initialize_model=True):
        ...
        self.goal_locations = [
            [7, 3]
        ]  # [[6,2]] a vector specifying the goal locations in [[x1,y1],[x2,y2]] format
        self.goal_rewards = [
            100
        ]  # a vector specifying the associated rewards with the goals in self.goal_locations, in [r1,r2] format
    ...
    def step(self, a):
        ...
        self.agent_location += self.action_effects[
            a
        ]  # effect of action
        ...
        if (
            np.random.rand() < self.wind_blows_proportion
        ):  # apply effect of wind
            self.agent_location[1] += self.winds[
        ...
        s_next = self._location_to_state(
            self.agent_location
        )
        ...
        goal_present = np.any(
            [
                np.all(goal_location == self.agent_location)
        ...
        if goal_present:
            goal_index = np.where(
                [
                    np.all(
                        goal_location == self.agent_location
                    )
                    for goal_location in self.goal_locations
                ]
            )[0][0]
    ...
    def model(self, s, a):
        ...
        if self.initialize_model:
            ...
            raise ValueError(
                "set initialize_model=True when creating Environment"
            )
        ...
        if Q_sa is not None:
            ...
            for state in range(self.n_states):
                for action in range(self.n_actions):
                    self.Q_labels[state][action].set_text(
                        np.round(Q_sa[state, action], 1)
                    )
    ...
    def _construct_model(self):
        ...
        )  # set all rewards to the default value
        ...
        for s in range(self.n_states):
            for a in range(self.n_actions):
                ...
                state_is_a_goal = np.any(
                    [
                        np.all(goal_location == s_location)
                        for goal_location in self.goal_locations
                    ]
                )
                if state_is_a_goal:
                    ...
                    next_location_with_wind = np.copy(
                        s_location
                    )
                    next_location_with_wind += (
                        self.action_effects[a]
                    )  # effect of action
                    next_location_with_wind = np.clip(
                        next_location_with_wind,
                        (0, 0),
                        np.array(self.shape) - 1,
                    )  # bound within grid
                    next_location_with_wind[1] += (
                        self.winds[
                            next_location_with_wind[0]
                        ]
                    )  # Apply effect of wind
                    next_location_with_wind = np.clip(
                        next_location_with_wind,
                    ...
                    next_state_with_wind = (
                        self._location_to_state(
                            next_location_with_wind
                        )
                    )

                    # Update p_sas and r_sas
                    p_sas[s, a, next_state_with_wind] += (
                        self.wind_blows_proportion
                    )
                    for i, goal in enumerate(
                        self.goal_locations
                    ):
                        if np.all(
                            next_location_with_wind == goal
                        ):  # reached a goal!
                            r_sas[
                                s, a, next_state_with_wind
                            ] = self.goal_rewards[i]

                    # check what happens if the wind does not blow:
                    next_location_without_wind = np.copy(
                        s_location
                    )
                    next_location_without_wind += (
                        self.action_effects[a]
                    )  # effect of action
                    next_location_without_wind = np.clip(
                        next_location_without_wind,
                        (0, 0),
                        np.array(self.shape) - 1,
                    )  # bound within grid
                    next_state_without_wind = (
                        self._location_to_state(
                            next_location_without_wind
                        )
                    )
                    ...
                    # Update p_sas and r_sas
                    p_sas[
                        s, a, next_state_without_wind
                    ] += 1 - self.wind_blows_proportion
                    for i, goal in enumerate(
                        self.goal_locations
                    ):
                        if np.all(
    ...
    def _initialize_plot(self):
        self.fig, self.ax = (
            plt.subplots()
        )  # figsize=(self.width, self.height+1)) # Start a new figure
        ...
        self.ax.axvline(
            0, 0, self.height, linewidth=5, c="k"
        )
        self.ax.axvline(
            self.width, 0, self.height, linewidth=5, c="k"
        )
        self.ax.axhline(
            0, 0, self.width, linewidth=5, c="k"
        )
        self.ax.axhline(
            self.height, 0, self.width, linewidth=5, c="k"
        )
        ...
        self.ax.text(
            self.start_location[0] + 0.05,
            self.start_location[1] + 0.75,
            "S",
            fontsize=20,
            c="b",
        )
        ...
        for i in range(len(self.goal_locations)):
            ...
            self.ax.text(
                self.goal_locations[i][0] + 0.05,
                self.goal_locations[i][1] + 0.75,
                text,
                fontsize=20,
                c=colour,
            )
        ...
        for state in range(self.n_states):
            ...
            for action in range(self.n_actions):
                plot_location = (
                    ...
                    + 0.42
                    + 0.35
                    * np.array(self.action_effects[action])
                )
                next_label = self.ax.text(
                    plot_location[0],
    ...
    def _plot_arrows(self, Q_sa):
        ...
        for state in range(self.n_states):
            plot_location = (
                np.array(self._state_to_location(state))
                + 0.5
            )
            ...
            for max_action in max_actions:
                new_arrow = arrow = Arrow(
                    plot_location[0],
                    plot_location[1],
                    self.action_effects[max_action][0]
                    * 0.2,
                    self.action_effects[max_action][1]
                    * 0.2,
                    width=0.05,
                    color="k",
                )
