...
def average_over_repetitions(
    ...
    print(
        "Running one setting takes {} minutes".format(
            (time.time() - now) / 60
        )
    )
...
def experiment():
    ...
    for epsilon in tqdm(epsilons, desc="Epsilons"):
        ...
        Plot.add_curve(
            timesteps,
            learning_curve,
            label=r"$\epsilon$-greedy, $\epsilon $ = {}".format(
                epsilon
            ),
        )
    policy = "softmax"
